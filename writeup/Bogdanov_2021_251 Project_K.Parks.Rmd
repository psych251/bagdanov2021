...
bogdanovtidy$TSST_difficult_control <- as.numeric(as.factor(bogdanovtidy$TSST_difficult_control))
...
str(DST_data[,c(1:10)]) #to check if .CSV data has been read as boolean (T/F), intergers or characters 
...
DSTeffort <- DST_data |>
 ...
 summarize(cnt = n()) |>
  mutate(freq = round(cnt / sum(cnt), 3)*100) |> #to change rates into percentage
  mutate(percentage = freq)
DSTeffort

DSTeffort2 <- DSTeffort[-c(4)] #remove freq column

LowDemandDST <- DSTeffort2 |> #only include low-demand choices ("1")
  filter(effort_choice == 1)
...
```{r}
library(knitr)
knitr::include_graphics("bogdanov_fig3.png")
```
...
ggpLD <- ggplot(data = LowDemandDST, aes(x=condition, y = percentage, fill=condition)) +
 ...
 labs(x="Condition", y="Low-Demand Choices (%)", title = "Percentage of Low-Demand Choices Across Trials") +
